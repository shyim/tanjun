...
func (h *hijackedIOStreamer) setupInput() (restore func(), err error) {
	...
	if err := setRawTerminal(h.in, h.out); err != nil {
	...
	restore = func() {
		restoreOnce.Do(func() {
			_ = restoreTerminal(h.in, h.out, h.inputStream)
		})
	}
...
func setRawTerminal(in *streams.In, out *streams.Out) error {
...
func restoreTerminal(streamIn *streams.In, streamOut *streams.Out, in io.Closer) error {
	streamIn.RestoreTerminal()
	streamOut.RestoreTerminal()
